...
def main():
    ...
    parser.add_argument('--output_feature', type=int, help='Name for the output feature', required=True)
    parser.add_argument('--output', type=str, help='Path for the output file', required=True)
    parser.add_argument('--renderconfig', action='store_true', help='Render the config', required=False, default=False)
    ...
    args_init = ["--dataset", args.dataset, "--target", target, "--output", args.output]
